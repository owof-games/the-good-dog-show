...
-> lounge_loop_with_letter(bebe_tunnel, ugoemimi_tunnel, piiiietro_tunnel, quello_tunnel, ildivo_tunnel, kitchen_divert, -> invalid_letter)

= invalid_letter

    ~ throwException("Nessuna lettera disponibile")
    -> END


=== lounge_loop_with_letter(-> bebe_tunnel, -> ugoemimi_tunnel, -> piiiietro_tunnel, -> quello_tunnel, -> ildivo_tunnel, -> kitchen_divert, -> letter_tunnel)
...
{ alive_characters:
    ...
    + -> kitchen_divert
- else:
    // no alive characters: read the letter and go to the kitchen
    -> letter_tunnel ->
    -> kitchen_divert
}
